    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {mockDraft.picks.map((pick) => {
          ...
          <div
            key={pick.pick}
            className="bg-white rounded-lg p-4 border border-gray-200 hover:border-rose-200 shadow-sm transition-colors"
          >
            <div className="flex items-start gap-3">
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                <span className="w-8 h-8 bg-rose-500 text-white rounded-lg flex items-center justify-center font-bold text-sm">
                  {pick.pick}
                </span>
                <TeamLogo teamName={pick.team} size={36} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium text-gray-700">{pick.team}</div>
                {needs.length > 0 && (
                ...
                {pick.notes && (
                  <p className="mt-2 text-sm text-gray-500 line-clamp-2">{pick.notes}</p>
                )}
              </div>
            </div>
          </div>
